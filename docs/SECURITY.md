...
`govulncheck` is required for release readiness as a supply-chain gate. Nexdev pins the fixed release Go version in `go.mod` and CI/release workflows as `1.26.4`. `govulncheck` availability is an environment requirement; Nexdev does not implement vulnerability scanning as product runtime behavior.
