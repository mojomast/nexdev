...
`govulncheck` is required to be present on `PATH`; the script fails with exit code `127` if it is unavailable. The script does not install tools and does not run real-provider/network tests.
